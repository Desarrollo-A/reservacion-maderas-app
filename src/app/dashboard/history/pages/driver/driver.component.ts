...
export class DriverComponent implements OnInit, AfterViewInit {
  ...
  getData(): Observable<DriverModel[]> {
    return of<DriverModel[]>([
      {
        fullName: 'Ramon Torres Gonzalez',
        date: addHours(startOfDay(new Date()), 8),
        nameApplicant: 'Juan Gonzalez Rendon',
        availability: <Lookup>{name: 'Disponible'}
      },
      {
        fullName: 'Pedro Perez León',
        date: addHours(startOfDay(new Date()), 8),
        nameApplicant: 'Roberto Carlos Azul',
        availability: <Lookup>{name: 'Disponible'}
      },
      {
        fullName: 'Pablo Perez León',
        date: addHours(startOfDay(new Date()), 8),
        nameApplicant: 'Juan Gonzalez Rendon',
        availability: <Lookup>{name: 'Reservado'}
      },
      {
        fullName: 'German Rodríguez Paes',
        date: addHours(startOfDay(new Date()), 8),
        nameApplicant: 'José María Napoleón',
        availability: <Lookup>{name: 'Disponible'}
      },
      {
        fullName: 'Edgar Teran Estrada',
        date: addHours(startOfDay(new Date()), 8),
        nameApplicant: 'Marco Antonio Solis',
        availability: <Lookup>{name: 'Reservado'}
      },
